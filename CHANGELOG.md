...
- **Bumped `@quillmark/wasm` 0.54.1 → 0.66.1**. The wasm engine moved from a per-instance quill registry to a stateless factory: `engine.quill(tree) → Quill`, with rendering on `quill.render(doc, opts?)` and parsing on `Document.fromMarkdown(content)`. Engine-level `registerQuill`, `dryRun`, `parseMarkdown`, `getQuillInfo`, and `getQuillSchema` are gone.
...
- Composable card definitions live under `card_types:` (was `cards:`).
...
- Quill-level `description:` (under `quill:`) now surfaces at `metadata.description` (since wasm 0.66.1) — independent of `metadata.schema.main.description` (the schema description of the entry-point card).
