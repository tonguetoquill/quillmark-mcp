...
| `--concurrency N` | `4` | Concurrent runs across the matrix |
...
parallel. Default is 4 — gentle on rate limits, ~4x faster than serial.

Tasks are ordered `trial -> prompt -> model` so adjacent tasks differ in
model: with 4 workers across 6 providers, you tend to hit 4 distinct
providers at once rather than hammering one. The single MCP client is
shared across workers (JSON-RPC ids match responses to requests, so
concurrent `callTool`s are safe).

Dial it up if your providers can take it (`--concurrency 8` or `16`),
