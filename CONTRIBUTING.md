...
- **[API Reference](https://nibsbin.github.io/quillmark-mcp/)** — auto-generated by TypeDoc from JSDoc annotations. One page per module/class/function with cross-linked navigation, sidebar, search, and dark/light toggle. Deployed to GitHub Pages on every push to main (`.github/workflows/docs.yml`). Build locally: `npm run docs:build` → open `docs-site/index.html`.
- **[GitHub Wiki](https://github.com/nibsbin/quillmark-mcp/wiki)** — 18 hand-written pages covering architecture, CLI, tools, strategies, quills, Docker, testing, security, and more. Edit via the wiki web UI.

**When adding new source files**: add JSDoc annotations (`@module`, `@param`, `@returns`, `@throws`). TypeDoc picks them up automatically on next push to main. Use JSDoc Closure-style types (`{Function}`, `{object}`, `{string}`) — not TypeScript arrow syntax. See existing annotations for examples.
